        ...
        var that = {
            ...
            redraw: function() {
                ...
                sys.eachNode(function(node, pt) {
                    var w = Math.max(50, 50 + gfx.textWidth(node.name))
                    if (node.data.alpha === 0) return
                    if (node.data.shape == 'dot') {
                        gfx.oval(pt.x - w / 2, pt.y - w / 2, w, w, { fill: node.data.color, alpha: node.data.alpha })
                        gfx.text(node.name, pt.x, pt.y + 7, { color: "white", align: "center", font: "Arial", size: 17 })
                        gfx.text(node.name, pt.x, pt.y + 7, { color: "white", align: "center", font: "Arial", size: 17 })
                    } else {
                        gfx.rect(pt.x - w / 2, pt.y - 8, w, 30, 8, { fill: node.data.color, alpha: node.data.alpha })
                        gfx.text(node.name, pt.x, pt.y + 15, { color: "white", align: "center", font: "Arial", size: 15 })
